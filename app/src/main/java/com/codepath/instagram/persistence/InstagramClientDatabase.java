...
public class InstagramClientDatabase extends SQLiteOpenHelper {
    ...
    public void emptyAllTables() {
        ...
        try {
            ...
            Log.wtf(TAG, "Error while trying to empty all tables in the database");
            e.printStackTrace();
        } finally {
            db.endTransaction();
        }
    ...
    public void addInstagramPosts(ArrayList<InstagramPost> posts) {
        if (posts == null) {
            throw new IllegalArgumentException(String.format("Attempting to add a null list of posts to %s", DATABASE_NAME));
        }
                ...
                do {
                    ...
                    try {
                        if (commentsCursor.moveToFirst()) {
                            ...
                            post.comments = new ArrayList<InstagramComment>();
                        }
                    } catch (Exception e) {
